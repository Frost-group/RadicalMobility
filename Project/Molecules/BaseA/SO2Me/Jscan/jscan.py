...
def optimize_geometry(xyz_file, charge, uhf, log_file):
    ...
    result = subprocess.run(
        ['xtb', xyz_file, '--opt', '--chrg', str(charge), '--uhf', str(uhf), '--gfn 2'],
...
def optimize_geometry_gfnff(xyz_file, charge, uhf, log_file):
    optimized_xyz = xyz_file.replace("initial", "optimized")
    result = subprocess.run(
...
if __name__ == "__main__":
    charge, uhf = 0, 2
    input_mol_file = "loose_opt_dimer.xyz"
    ...
    with open("calculation_log.txt", "w") as log_file:
        ...
        #loose_optimized_xyz = optimize_geometry_gfnff(initial_xyz, charge, uhf, "calculation_log.txt")
        #optimized_xyz = optimize_geometry(loose_optimized_xyz, charge, uhf, "calculation_log.txt")

        # Scan distances around optimized structure
        scan_range = np.arange(0, 4.0, 0.05)
        results = []

        for delta in scan_range:
            scan_distance = initial_distance + delta
            scan_xyz_file = f"dimer_scan_{scan_distance:.2f}.xyz"
            translate_and_save(input_mol_file, delta, scan_xyz_file)
