...
def optimize_geometry_gfnff(input_xyz_file, output_mol_file, charge, uhf, log_file_path):
    with open(log_file_path, 'a') as log:
        ...
        subprocess.run(['xtb', input_xyz_file, '--opt', '--gfnff', '--chrg', str(charge), '--uhf', str(uhf)], 
                       check=True, stdout=log, stderr=log)
...
def calculate_transfer_integral(xyz_file, charge, uhf, log_file_path):
    result = subprocess.run(
        ['xtb', xyz_file, '--dipro', '0.3', '--chrg', str(charge), '--uhf', str(uhf), '--spinpol', '--tblite', '--input', 'fragment.inp'],
        check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
    )
