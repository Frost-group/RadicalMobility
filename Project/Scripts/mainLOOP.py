...
def optimize_geometry(input_mol_file, output_mol_file, charge, uhf, log_file_path):
    with open(log_file_path, 'a') as log:
        subprocess.run(['xtb', input_mol_file, '--opt', '--chrg', str(charge), '--uhf', str(uhf), '--tight', '--spinpol', '--tblite'], 
...
def run_calculations(input_mol_file, log_txt_path, log_csv_path):
    ...
    with open(log_csv_path, 'a', newline='') as file:  # Append data to CSV
        ...
        opt_radical_monomer = optimize_geometry(input_mol_file, 'radical_monomer.mol', radical_monomer_charge, radical_monomer_uhf, log_txt_path)
        opt_anion_monomer = optimize_geometry(input_mol_file, 'anion_monomer.mol', anion_monomer_charge, anion_monomer_uhf, log_txt_path)
        opt_cation_monomer = optimize_geometry(input_mol_file, 'cation_monomer.mol', cation_monomer_charge, cation_monomer_uhf, log_txt_path)
        reorg_energy_electron, reorg_energy_hole = calculate_reorganization_energy(opt_radical_monomer, opt_anion_monomer, opt_cation_monomer, radical_monomer_charge, radical_monomer_uhf, anion_monomer_charge, anion_monomer_uhf, cation_monomer_charge, cation_monomer_uhf, log_txt_path)
        
        for dx in range(4, 4, 1):
            for dy in range(4, 4, 1):
                for dz in range(4, 4, 1):
                    for rx in range(0, 0, 1):
                        for ry in range(0, 0, 1):
                            for rz in range(0, 0, 1):
                                ...
                                loose_opt_dimer = optimize_geometry_gfnff(dimer, 'loose_opt_dimer.mol', radical_dimer_charge, log_txt_path)
                                opt_dimer = optimize_geometry(loose_opt_dimer, 'opt_dimer.mol', radical_dimer_charge, radical_dimer_uhf, log_txt_path)
                                
                                J_hole, J_electron = calculate_transfer_integral(opt_dimer, radical_dimer_charge, radical_dimer_uhf, log_txt_path)
...
if __name__ == "__main__":
    run_calculations("Base1tBu.mol", "output_log.txt", "molecular_data_log.csv")
